...
static void display_symbol_table(Elf64_Ehdr *ehdr, Elf64_Shdr *shdr, Elf64_Shdr *symtab);
static unsigned char get_symbol_letter(Elf64_Sym symbol, Elf64_Shdr *shdr);
static Elf64_Shdr* get_section_headers_table(Elf64_Ehdr *ehdr);
static Elf64_Shdr* find_section_by_type(Elf64_Shdr *shdrtab, uint16_t shnum, uint32_t type);
static Elf64_Shdr *get_symbol_table(Elf64_Shdr *shdrtab, Elf64_Ehdr *ehdr);
static int should_skip_symbol(const char *symbol_name, Elf64_Sym symbol);
static void check_ELF_file_integrity(void *binary_pointer, off_t size);
static void check_symtab_integrity(const Elf64_Ehdr *elf_header,
							const Elf64_Shdr *section_headers,
							const Elf64_Shdr *symbol_table);
static void check_section_headers_table_integrity(Elf64_Ehdr *ELF_header, Elf64_Shdr *section_header_table);
static void print_symbol(Elf64_Shdr *section_headers, unsigned long long int index);
...
void display_symbol_table(Elf64_Ehdr *elf_header, Elf64_Shdr *section_headers, Elf64_Shdr *symbol_table) {
	...
	symbol_array64 = (Elf64_Sym *) ((char *) elf_header + symbol_table->sh_offset);
	...
	if (G_FLAGS[NO_SORT_FLAG]) {
		...
		print_btree64(g_master_node, section_headers);
	}
...
void print_btree64(t_btree *node, Elf64_Shdr *sdhdrtab) {
	if (node->left) {
		print_btree64(node->left, sdhdrtab);
	}
	unsigned long long index = node->index;
	print_symbol(sdhdrtab, index);
	if (node->right) {
		print_btree64(node->right, sdhdrtab);
	}
}

void print_symbol(Elf64_Shdr *section_headers, unsigned long long int index) {
	char *symbol_name = symtab_strtab_ptr + symbol_array64[index].st_name;
